"""Subprocess-driven e2e CLI tests for postino.

Every test invokes the installed `postino` console script against a real
MariaDB schema seeded with two domains, six mailboxes, two aliases, and
PostfixAdmin's 'ALL' pseudo-row (the production regression trigger for the
domain list crash).

Design decisions:
- subprocess.run against the installed `postino` entry-point: exercises the
  full console-script registration path and import chain. Falls back to
  `python -m postino.cli` via typer if no __main__ is present.
- Module-scoped DB fixture: read-only commands share seed data; tests that
  would mutate DB state add/clean up their own rows.
- POSTINO_* env vars drive settings (same mechanism as existing CLI tests);
  no TOML file or POSTINO_CONFIG override needed.
...
def test_postino_domain_list_json_output(e2e_env: dict[str, str]) -> None:
    """JSON output mode: parses cleanly, both domains present, ALL absent."""
    code, out, err = _run(["--json", "domain", "list"], e2e_env)
    assert code == 0, f"stderr: {err}"
    data = _json_list(out)
