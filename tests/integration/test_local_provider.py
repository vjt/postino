...
def _seed_mailbox(conn, md: MetaData, username: str) -> None:
    mailbox = md.tables["mailbox"]
    domain = md.tables["domain"]
    conn.execute(domain.insert().values(
        domain="example.com",
        description="",
        aliases=0,
        mailboxes=0,
...
def test_local_create_identity_writes_password(db: Engine) -> None:
    ...
    with db.begin() as conn:
        ...
        row = conn.execute(
            select(md.tables["mailbox"].c.password)
            .where(md.tables["mailbox"].c.username == "foo@example.com")
        ).scalar_one()
...
def test_local_set_password_overrides(db: Engine) -> None:
    ...
    with db.begin() as conn:
        ...
        prov = LocalProvider(metadata=md)
        prov.create_identity(
            conn, "foo@example.com", name="", password=SecretStr("a"),
            scheme=PasswordScheme.BCRYPT,
        )
        prov.set_password(
            conn, "foo@example.com", SecretStr("b"), PasswordScheme.BCRYPT
        )
        row = conn.execute(
            select(md.tables["mailbox"].c.password)
            .where(md.tables["mailbox"].c.username == "foo@example.com")
...
def test_local_set_password_missing_raises(db: Engine) -> None:
    ...
    with db.begin() as conn:
        ...
        with pytest.raises(NotFoundError):
            prov.set_password(
                conn, "ghost@example.com", SecretStr("x"), PasswordScheme.BCRYPT
            )
