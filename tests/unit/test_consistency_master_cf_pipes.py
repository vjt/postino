...
Validates that master.cf has all 5 v0.10 mlmmj pipe service blocks.
"""
...
_ALL_FIVE = (
    ...
    f"mlmmj-help unix - n n - - pipe\n"
    f"   flags=DRhu user=mlmmj argv=/usr/bin/mlmmj-help -L {_SPOOL}\n"
)
...
def test_check_lists_missing_master_cf_pipes(tmp_path: Path) -> None:
    ...
    assert any("mlmmj-help" in m for m in missing_names)


def test_check_passes_when_all_five_pipes_present(tmp_path: Path) -> None:
    master_cf = tmp_path / "master.cf"
