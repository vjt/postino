...
def test_dispatch_local() -> None:
    p = _provider_for(IdentityBackend.LOCAL, metadata=MetaData(), clock=_clock)
    assert isinstance(p, LocalProvider)


def test_dispatch_noauth() -> None:
    p = _provider_for(IdentityBackend.NOAUTH, metadata=MetaData(), clock=_clock)
    assert isinstance(p, NoAuthProvider)


def test_dispatch_hybrid() -> None:
    p = _provider_for(IdentityBackend.HYBRID, metadata=MetaData(), clock=_clock)
